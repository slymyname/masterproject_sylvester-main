...
class hydraulicMesh:
    ...
    def adj2path(self, G):
        ...
        if np.all(G == 0):
            ...
            L = np.zeros(int(np.sum(np.sum(G + np.multiply(G, np.eye(G.shape[0]))) / 2)),
                         dtype=int)  # Counts number of vertices
    ...
    def MeshPrep(self, StaticData):
        ...
        for i in range(len(cycles)):
            for j in range(len(cycles[i]) - 1):
                cycles_edges[i][j] = np.where(np.logical_and(A[cycles[i][j], :] != 0, A[cycles[i][j + 1], :] != 0))[0][
                    0]
        ...
        for l in leftEdgesAdjDir:
            ...
            for i in range(len(temprows)):
                leftEdges = leftEdges + (
                np.where(np.logical_and(np.isin(startnode, tempcols[i]), np.isin(targetnode, temprows[i])))[0]).tolist()
