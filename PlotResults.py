...
class PlotResult:
    ...
    def __init__(self,ProgramModes):
        ...
        self.HL2=[]
        self.PumpPower=[]
        self.ProgramModes = ProgramModes
        self.T_supply = []
        self.p_plant = []
    ...
    def plot_combined(self, TimeSeries, ProgramModes):
        if self.ProgramModes.SimulationEngine == 'ROM':
            fig, axes = plt.subplots(2, 2, figsize=(15, 12), sharex=True)  # Create 2x2 subplots for ROM
            ax = axes.ravel()

            self.plotT(ax[0], TimeSeries, ProgramModes)
            self.plotp(ax[1], TimeSeries, ProgramModes)
            self.plotm_i(ax[2], TimeSeries, ProgramModes)
            self.plotHeatLosses(ax[3], TimeSeries, ProgramModes)

            ax[0].set_xlabel('Time')
            ax[2].set_xlabel('Time')

        else: # Keep original for FOM
            fig, axes = plt.subplots(3, 2, figsize=(15, 12), sharex=True)  # Create 3x2 subplots
            ax = axes.ravel()
            self.plotT(ax[0], TimeSeries, ProgramModes)
            self.plotp(ax[1], TimeSeries, ProgramModes)
            self.plotm_i(ax[2], TimeSeries, ProgramModes)
            ...
            # Plot 6: Total Heat Losses (sum(Q_loss_Pipe)) over time
            plot_datetimes_hl = []
            if len(TimeSeries.datetimeStationary) > 0:
                start_time_main_loop = TimeSeries.datetimeStationary[0]
                if ProgramModes.TempratureCalculationMode == 'stationary' or ProgramModes.TempratureCalculationMode == 'default':
                    if start_time_main_loop == ProgramModes.startTime + datetime.timedelta(microseconds=1):
                        plot_datetimes_hl.append(ProgramModes.startTime) # Plot first point at actual start time
                        if len(self.HL1) > 1:
                             plot_datetimes_hl.extend(TimeSeries.datetimeStationary[1:])
                    else:
                        plot_datetimes_hl.extend(TimeSeries.datetimeStationary)
                else: 
                    plot_datetimes_hl.extend(TimeSeries.datetimeStationary)
            
            if not plot_datetimes_hl or len(plot_datetimes_hl) != len(self.HL1):
                print("[PlotResults] Warning: Datetime alignment for HL1 plot might be off. Using direct sequence if necessary.")
                if len(self.HL1) > 0:
                     plot_datetimes_hl = TimeSeries.datetimeStationary[:len(self.HL1)]
                     if not plot_datetimes_hl : # if still empty
                         plot_datetimes_hl = list(range(len(self.HL1)))

            ax[5].plot(plot_datetimes_hl, [val/1000 for val in self.HL1], label='Total Instantaneous Heat Loss', color='purple')
            ax[5].set_xlabel('Time')
            ax[5].set_ylabel('Total Heat Loss (kW)')
            ax[5].set_title('Total Instantaneous Heat Loss Profile')
            ax[5].legend()
            ax[5].grid(True)
