...
class TimeSeries:
    ...
    def updateQ(self,StaticData,ProgramModes):
        ...
        for n in range(StaticData.nbOutlets):
            if self.denom[n]==0:
                self.denom[n]=StandardConsumptionProfile.prepareConsumption(self,StaticData.Outlets_ConsumerType[n])
            T_weigth=(self.T_mean+0.5*self.T_mean_yesterday)/1.5
            self.Q[n]=StandardConsumptionProfile.calcConsumption(self,StaticData.Outlets_ConsumerType[n], self.denom[n],StaticData.Outlets_Qa[n],T_weigth,self.datetimeStationary[-1].weekday(),self.datetimeStationary[-1].hour)
